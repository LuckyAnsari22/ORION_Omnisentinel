...
class Movenet(AbstractPoseModel):
    ...
    def parse_output(self, keypoints_with_scores, height, width):
        ...
        for idx in range(num_instances):
            # MoveNet output is [y, x, score] (standard TFLite format)
            kpts_y = keypoints_with_scores[0, idx, :, 0]
            kpts_x = keypoints_with_scores[0, idx, :, 1]
            kpts_scores = keypoints_with_scores[0, idx, :, 2]
            
            # Reverting to Y, X order specifically to match what PoseEngine/PoseNet expect
            kpts_absolute_yx = np.stack([height * kpts_y, width * kpts_x, kpts_scores], axis=-1)
            keypoints_all.append(kpts_absolute_yx)
    ...
    def execute_model(self, img):
        ...
        if floating_model:
            # MoveNet singlepose thunder float models expect [0, 1]
            template_input = template_input.astype(np.float32) / 255.0
